#!/usr/bin/env python3
"""
林小雨「気持ちを伝える中国語フレーズ50」改良版 PDF生成
KDP出版対応 A5版
"""
import math, os, re
...
# ── FONTS ──────────────────────────────────────────────────────────────────
# KDP requires all fonts to be embedded. CID fonts (Heisei*, STSong-Light) are
# NOT embedded by reportlab — they assume the viewer has the matching CMap.
# All fonts below are TTF, which reportlab automatically embeds.
from reportlab.pdfbase.ttfonts import TTFont

def _find_font(candidates, install_hint):
    for p in candidates:
        if os.path.exists(p):
            return p
    raise FileNotFoundError(f"Font not found. Tried:\n  " + "\n  ".join(candidates) + f"\n{install_hint}")

NOTO_SANS_JP  = _find_font([
    os.path.expanduser("~/Library/Fonts/NotoSansJP[wght].ttf"),
    "/Library/Fonts/NotoSansJP[wght].ttf",
], "Install: brew install --cask font-noto-sans-jp")

NOTO_SERIF_JP = _find_font([
    os.path.expanduser("~/Library/Fonts/NotoSerifJP[wght].ttf"),
    "/Library/Fonts/NotoSerifJP[wght].ttf",
], "Install: brew install --cask font-noto-serif-jp")

NOTO_SANS_SC  = _find_font([
    os.path.expanduser("~/Library/Fonts/NotoSansSC[wght].ttf"),
    "/Library/Fonts/NotoSansSC[wght].ttf",
], "Install: brew install --cask font-noto-sans-sc")
...
], "Install: brew install --cask font-dejavu")

# STHeiti subset retained for legacy fallback (now superseded by NotoSansSC,
# but kept embedded in case of future mixed-text edge cases).
STHEI_SUBSET = os.path.join(os.path.dirname(__file__), 'fonts', 'STHeitiSC-Medium-subset.ttf')

pdfmetrics.registerFont(TTFont('NotoSansJP',  NOTO_SANS_JP))   # Japanese Gothic body
pdfmetrics.registerFont(TTFont('NotoSerifJP', NOTO_SERIF_JP))  # Japanese Mincho headings
pdfmetrics.registerFont(TTFont('NotoSansSC',  NOTO_SANS_SC))   # Simplified Chinese
pdfmetrics.registerFont(TTFont('STHeitiSC-GB', STHEI_SUBSET))  # Legacy subset (kept for safety)
pdfmetrics.registerFont(TTFont('DejaVu',      DEJAVU_SANS))    # Pinyin Latin (tone marks)

# Override reportlab's default Helvetica (Type 1, not embedded) with a TTF.
# KDP rejects PDFs containing non-embedded Standard 14 fonts; any stray
# default-font reference in reportlab's internals now resolves to embedded NotoSansJP.
pdfmetrics.registerFont(TTFont('Helvetica', NOTO_SANS_JP))

F_CH    = 'NotoSansSC'    # Main Chinese phrase display
F_CH_GB = 'NotoSansSC'    # Simplified-Chinese chars in mixed text (was STHeitiSC-GB)
F_JA    = 'NotoSansJP'    # Japanese body (Gothic)
F_JA_M  = 'NotoSerifJP'   # Japanese elegant/headings (Mincho/Serif)
F_PY    = 'DejaVu'        # Pinyin (full Unicode Latin for tone marks)

# ── COLORS ──────────────────────────────────────────────────────────────────
C_BG         = HexColor('#FFFBFC')
C_PINK_LIGHT = HexColor('#FFE8EF')
C_PINK_MED   = HexColor('#F5C2CC')
C_PINK_DARK  = HexColor('#E8829A')
C_ROSE       = HexColor('#C4637A')
C_DARK_ROSE  = HexColor('#8B3A52')
C_BROWN      = HexColor('#3D1C1C')
C_BROWN_MED  = HexColor('#6B3A3A')
C_LINE       = HexColor('#DEB4BE')
C_BOX_BG     = HexColor('#FFF0F4')
C_BOX_BD     = HexColor('#E8B4BF')
C_CHAPTER_BG = HexColor('#E8A0B0')
C_WHITE      = white
...
OUTPUT = os.path.join(os.path.dirname(__file__), 'output', 'lin_phrases.pdf')

# ── DATA ────────────────────────────────────────────────────────────────────
CHAPTERS = [
    {
        "num": 1, "title": "出会いのフレーズ",
        "quote": "最初の一言が、全部を決める",
        "phrases": [
            (1, "你好！", "Nǐ hǎo!", "こんにちは！",
             "中国語の基本中の基本だよ。でもね、発音が大事。「ニーハオ」って棒読みにならないように。「你（nǐ）」は3声、「好（hǎo）」も3声。2つ続くと前が2声に変わるの。ní hǎoって感じ。声に出してみて！"),
            (2, "我叫林小雨，你叫什么名字？",
             "Wǒ jiào Lín Xiǎoyǔ, nǐ jiào shénme míngzi?",
             "私は林小雨といいます。あなたのお名前は？",
             "自己紹介の定番。「叫（jiào）」は「〜と呼ばれる」って意味。名前の部分を自分の名前に変えるだけで使えるよ。恥ずかしがらずに言ってみて！"),
            (3, "很高兴认识你！", "Hěn gāoxìng rènshi nǐ!",
             "はじめまして、よろしく！",
             "直訳すると「あなたと知り合えてとても嬉しい」。日本語の「よろしく」より気持ちが込もってる表現だよ。初対面でこれを言うと、相手の表情が柔らかくなるの。試してみて！"),
            (4, "你是日本人吗？", "Nǐ shì Rìběnrén ma?",
             "あなたは日本人ですか？",
             "「吗（ma）」をつけると疑問文になるの。すごく便利でしょ？「是（shì）」の部分を変えれば色々な質問ができるよ。「你是学生吗？（学生ですか？）」みたいに。"),
            (5, "我是来日本留学的。", "Wǒ shì lái Rìběn liúxué de.",
             "私は日本に留学に来ています。",
             "これ、私が毎日使ってる文だよ（笑）。「来〜的」で「〜しに来た」って言えるの。「来日本旅行的（旅行に来た）」「来日本工作的（仕事で来た）」に応用できるよ！"),
        ]
    },
    {
        "num": 2, "title": "気持ちを伝える",
        "quote": "感情を言葉にできると、中国語が楽しくなるよ",
        "phrases": [
            (6,  "太好了！", "Tài hǎo le!", "やった！／最高！",
             "私が一番よく使う言葉かも。「太（tài）」は「すごく・とても」って意味。「好（hǎo）」は「良い」。合わせると「最高すぎる！」って感じ。テスト合格した時、好きなものを食べた時、なんでも使えるよ！"),
            (7,  "我很开心！", "Wǒ hěn kāixīn!", "すごく嬉しい！",
             "「开心（kāixīn）」は「心が開く」って書くの。素敵じゃない？嬉しい時、楽しい時に使ってね。「你让我很开心（あなたのおかげで嬉しい）」って言えたら最高だよ。"),
            (8,  "真的吗？！", "Zhēn de ma?!", "本当に？！",
             "驚いた時の定番。「真的（zhēn de）」は「本当に」。びっくりした時、信じられない時、嬉しいニュースを聞いた時。日常会話でめちゃくちゃ使うよ。リアクション上手になれる魔法の言葉！"),
            (9,  "我有点儿难过。", "Wǒ yǒudiǎnr nánguò.", "ちょっと悲しい。",
             "「有点儿（yǒudiǎnr）」は「少し・ちょっと」。これをつけると感情を柔らかく伝えられるの。「难过（nánguò）」は「心が苦しい・悲しい」って意味。無理して元気なふりしなくていい時、使ってみて。"),
            (10, "没事，别担心。", "Méi shì, bié dānxīn.", "大丈夫、心配しないで。",
             "友達を励ます時の定番フレーズ。「没事（méi shì）」は「何でもない・大丈夫」、「别担心（bié dānxīn）」は「心配しないで」。誰かが落ち込んでる時、そっと言えたらかっこいいよね。"),
            (11, "我好紧张！", "Wǒ hǎo jǐnzhāng!", "めちゃくちゃ緊張してる！",
             "「紧张（jǐnzhāng）」は緊張・ドキドキ両方に使えるの。試験前でも、好きな人の前でも（笑）。「好（hǎo）」を感情の前につけると「すごく〜」って強調になるよ。"),
            (12, "我累了。", "Wǒ lèi le.", "疲れた。",
             "シンプルだけど、これが言えると一気に距離が縮まるの。「累（lèi）」は肉体的な疲れにも精神的な疲れにも使える。「今天好累（今日めちゃ疲れた）」もよく使うよ。"),
            (13, "压力好大。", "Yālì hǎo dà.", "プレッシャーがすごい。",
             "「压力（yālì）」はプレッシャー・ストレス。「大（dà）」は大きい。現代人みんな共感できる言葉だよね。私も論文の締め切り前によく言ってる（笑）。"),
            (14, "太可爱了！", "Tài kě'ài le!", "めちゃくちゃかわいい！",
             "「可爱（kě'ài）」はかわいい。人にも動物にも物にも使えるの。日本語の「かわいい」と同じ感覚で使っていいよ。これ言われると誰でも嬉しいから、積極的に使って！"),
            (15, "我有些不安。", "Wǒ yǒuxiē bù'ān.", "なんか不安だな。",
             "「不安（bù'ān）」は日本語と同じ意味。「有些（yǒuxiē）」は「少し・なんとなく」。完全に不安じゃないけど、モヤモヤしてる時にぴったりの表現。気持ちに正直でいることって大事だよ。"),
        ]
    },
    {
        "num": 3, "title": "好きを伝える",
        "quote": "好きって言葉、中国語だと恥ずかしくないよ",
        "phrases": [
            (16, "我喜欢你。", "Wǒ xǐhuan nǐ.", "あなたのことが好き。",
             "「喜欢（xǐhuan）」は「好き・好む」。「爱（ài）」より軽い好きの表現。友達にも、好きな人にも使えるよ。「我喜欢中文（中国語が好き）」みたいに物にも使えるの。まずここから練習してみて！"),
            (17, "你真的很棒！", "Nǐ zhēn de hěn bàng!", "あなた本当にすごい！",
             "「棒（bàng）」は「すごい・最高」って意味のスラング的な表現。褒め言葉の中でも特にテンションが上がる言葉。中国語が上手く言えた時、私がこれを言ったら素直に喜んでね（笑）。"),
            (18, "谢谢你一直陪着我。", "Xièxie nǐ yìzhí péizhe wǒ.",
             "ずっと一緒にいてくれてありがとう。",
             "「陪（péi）」は「寄り添う・一緒にいる」。単純な「ありがとう」より深い感謝の気持ちが伝わるの。大切な人に言えたら、絶対に喜ばれる。この本を読み終わったら、誰かに言ってみて。"),
            (19, "你笑起来真好看。", "Nǐ xiào qǐlái zhēn hǎokàn.", "笑顔が素敵だね。",
             "「笑起来（xiào qǐlái）」は「笑った時」、「好看（hǎokàn）」は「見た目が良い・素敵」。これ言われたら誰でも嬉しいよね。褒め上手になると、人間関係が変わるよ。"),
            (20, "我最喜欢你了。", "Wǒ zuì xǐhuan nǐ le.", "あなたのことが一番好き。",
             "「最（zuì）」は「一番・最も」。「喜欢」に「最」をつけるだけで気持ちが倍増する。「私のこと？」って顔されたら最高だよね。ドキドキしながら言ってみて！"),
            (21, "有你在真好。", "Yǒu nǐ zài zhēn hǎo.", "あなたがいてくれてよかった。",
             "「有你在（yǒu nǐ zài）」は「あなたがいる」。シンプルだけど、これが一番刺さる言葉だと私は思う。言葉より気持ちが大事、でも言葉にしないと伝わらない。勇気を出して言ってみて。"),
            (22, "我爱你。", "Wǒ ài nǐ.", "愛してる。",
             "中国語で一番重い愛の言葉。「喜欢」より何倍も深い意味がある。だから中国人はそう簡単に使わないの。でもね、言われた時の破壊力は最強。…私はまだ言ったことないけど（笑）。"),
            (23, "你对我很重要。", "Nǐ duì wǒ hěn zhòngyào.", "あなたは私にとって大切な人。",
             "「重要（zhòngyào）」は重要・大切。「对我（duì wǒ）」は「私にとって」。愛してるより言いやすくて、でも気持ちがしっかり伝わる。好きな人に言ってみたい言葉ナンバーワン。"),
            (24, "见到你我就高兴。", "Jiàn dào nǐ wǒ jiù gāoxìng.", "あなたに会うと嬉しくなる。",
             "「见到（jiàn dào）」は「会う・見かける」、「就（jiù）」は「〜すると自然に」。会うだけで嬉しくなる、そういう人が人生に何人かいたら幸せだよね。そういう人に言える言葉。"),
            (25, "你懂我。", "Nǐ dǒng wǒ.", "あなたは私のことをわかってくれる。",
             "「懂（dǒng）」は「理解する・わかる」。短い言葉だけど、これが言える関係って本当に特別。言葉が少なくても通じ合える感覚、中国語で言うと「默契（mòqì）」って言うの。あなたとも、そうなれたらいいな。"),
        ]
    },
    {
        "num": 4, "title": "本音を言う",
        "quote": "本音で話せる関係が、本当の友達だと思う",
        "phrases": [
            (26, "我想跟你说实话。", "Wǒ xiǎng gēn nǐ shuō shíhuà.", "本音を話したい。",
             "「说实话（shuō shíhuà）」は「本当のことを言う」。これを言える関係って、すごく大事。前置きとして使うと、相手も真剣に聞いてくれるよ。大切な話の始め方として覚えておいて。"),
            (27, "说实话，我不太喜欢。", "Shuō shíhuà, wǒ bú tài xǐhuan.",
             "正直に言うと、あまり好きじゃない。",
             "「不太（bú tài）」は「あまり〜ない」。「大嫌い」じゃなくて「あまり好きじゃない」って言える表現。角が立たないように本音を言う時に便利。日本人が苦手な「はっきり断る」練習にもなるよ。"),
            (28, "我有话想对你说。", "Wǒ yǒu huà xiǎng duì nǐ shuō.", "あなたに言いたいことがある。",
             "ドキッとする言葉でしょ（笑）。「话（huà）」は「言葉・話」。告白の前置きにも、真剣な相談にも使える。これを言われたら、ちゃんと向き合って聞いてあげてね。"),
            (29, "对不起，是我的错。", "Duìbuqǐ, shì wǒ de cuò.", "ごめん、私が悪かった。",
             "「对不起（duìbuqǐ）」は「ごめんなさい」、「我的错（wǒ de cuò）」は「私の間違い・私のせい」。謝る時は言い訳せずにこれだけ言えたら十分。シンプルな謝罪が一番伝わるの。"),
            (30, "我需要一点时间。", "Wǒ xūyào yìdiǎn shíjiān.", "少し時間が必要。",
             "「需要（xūyào）」は「必要とする」。すぐに答えが出せない時、気持ちの整理が必要な時に使って。「考えさせて」より柔らかく、でも真剣さが伝わる言葉。私もよく使うよ。"),
            (31, "我不知道怎么说。", "Wǒ bù zhīdào zěnme shuō.", "どう言えばいいかわからない。",
             "「怎么说（zěnme shuō）」は「どう言えばいいか」。言葉が見つからない時、これを正直に言えるのが一番誠実だと思う。中国語学習中も「怎么说？（なんて言うの？）」って使えるよ。"),
            (32, "你误会我了。", "Nǐ wùhuì wǒ le.", "誤解してる。",
             "「误会（wùhuì）」は「誤解」。日本語と同じ漢字だから覚えやすいでしょ。誤解されたまま黙ってるより、ちゃんと言葉にして解消した方がいい。関係を大切にしたい人にこそ使って。"),
            (33, "其实我一直想说。", "Qíshí wǒ yìzhí xiǎng shuō.", "実はずっと言いたかった。",
             "「其实（qíshí）」は「実は」、「一直（yìzhí）」は「ずっと」。ずっと心に抱えてたことを打ち明ける時の言葉。これが言えた時、関係が一段階深まると思う。勇気を出して。"),
            (34, "我有时候不太自信。", "Wǒ yǒu shíhou bú tài zìxìn.", "たまに自信がなくなる。",
             "「自信（zìxìn）」は日本語と同じ。「有时候（yǒu shíhou）」は「時々」。完璧じゃない自分を言葉にできることって、実は強さだと思う。弱さを見せられる関係こそ、本物だよ。"),
            (35, "我只是想让你知道。", "Wǒ zhǐshì xiǎng ràng nǐ zhīdào.", "ただ、知っててほしかっただけ。",
             "「只是（zhǐshì）」は「ただ〜だけ」、「让你知道（ràng nǐ zhīdào）」は「あなたに知ってもらう」。解決してほしいわけじゃない、ただ聞いてほしい時の言葉。これが言える関係、大切にしてね。"),
        ]
    },
    {
        "num": 5, "title": "別れ際のフレーズ",
        "quote": "またね、って言える関係が、一番好き",
        "phrases": [
            (36, "我舍不得你走。", "Wǒ shěbude nǐ zǒu.", "行ってほしくないな。",
             "「舍不得（shěbude）」は「手放したくない・別れがたい」。日本語にぴったりの訳がないくらい、深い感情の言葉。大切な人が帰る時、旅立つ時に使って。言われた人は絶対忘れられないよ。"),
            (37, "今天真的很开心。", "Jīntiān zhēn de hěn kāixīn.", "今日本当に楽しかった。",
             "別れ際に言える最高の言葉のひとつ。「今天（jīntiān）」は「今日」。楽しかった気持ちをちゃんと言葉にして伝えること、大事だよ。言われた方も絶対嬉しいから。"),
            (38, "下次再见面吧。", "Xià cì zài jiànmiàn ba.", "また今度会おうね。",
             "「下次（xià cì）」は「次回」、「见面（jiànmiàn）」は「会う」。「また会いましょう」の中国語版。「吧（ba）」をつけると「〜しようよ」って提案のニュアンスになるの。社交辞令じゃなく、本気で言ってね。"),
            (39, "保重身体。", "Bǎozhòng shēntǐ.", "体に気をつけてね。",
             "「保重（bǎozhòng）」は「大切にする」、「身体（shēntǐ）」は「体」。日本語の「お体に気をつけて」と同じ使い方。遠くに行く人、体調を崩してる人への別れの言葉として最高。心が伝わるよ。"),
            (40, "我会想你的。", "Wǒ huì xiǎng nǐ de.", "会いたくなると思う。",
             "「想（xiǎng）」は「会いたい・思う」の両方の意味がある。「会いたくなる」と「あなたのことを思う」が同時に言える言葉。別れ際にこっそり言えたら、相手の心に残るよ。"),
            (41, "随时联系我。", "Suíshí liánxì wǒ.", "いつでも連絡してね。",
             "「随时（suíshí）」は「いつでも」、「联系（liánxì）」は「連絡する」。これが言えるって、その関係がオープンだってこと。気軽に言えるようになると、人間関係がぐっと楽になるよ。"),
            (42, "一路平安。", "Yílù píng'ān.", "道中気をつけてね。",
             "旅立つ人への定番の言葉。「一路（yílù）」は「道のり」、「平安（píng'ān）」は「無事・平和」。旅行に行く友達、遠くに引っ越す人に言える言葉。温かい気持ちが伝わる表現だよ。"),
            (43, "别忘了我。", "Bié wàng le wǒ.", "私のこと忘れないでね。",
             "「别（bié）」は「〜しないで」、「忘了（wàng le）」は「忘れた」。シンプルだけど、これを言える関係って特別だと思う。冗談っぽく言っても、本気で言っても、どちらでも使えるよ。"),
            (44, "期待下次见面。", "Qīdài xià cì jiànmiàn.", "また会える日を楽しみにしてる。",
             "「期待（qīdài）」は日本語と同じ意味。次に会える日を楽しみにしてることを伝える言葉。これを言われると、相手も次に会う理由ができるの。関係を続けたい時に使って。"),
            (45, "今天谢谢你陪我。", "Jīntiān xièxie nǐ péi wǒ.", "今日一緒にいてくれてありがとう。",
             "「陪（péi）」は前にも出てきたね。覚えてる？「一緒にいる・寄り添う」って意味。時間を共有してくれたことへの感謝。これが自然に言えるようになったら、中国語だいぶ上手くなってるよ。"),
            (46, "好好照顾自己。", "Hǎohǎo zhàogu zìjǐ.", "自分を大切にしてね。",
             "「照顾（zhàogu）」は「世話をする・ケアする」、「自己（zìjǐ）」は「自分」。他人を大切にするのと同じくらい、自分を大切にしてほしい。これ、私が日本で一番学んだことかもしれない。"),
            (47, "有空来找我。", "Yǒu kòng lái zhǎo wǒ.", "暇な時会いに来てね。",
             "「有空（yǒu kòng）」は「暇がある・時間がある」、「找我（zhǎo wǒ）」は「私を探す・会いに来る」。気軽に誘える言葉。「いつでも来てね」より少しカジュアルで、友達っぽい表現だよ。"),
            (48, "我们下次一起去吃饭吧。", "Wǒmen xià cì yìqǐ qù chīfàn ba.",
             "今度一緒にご飯食べに行こう。",
             "「一起（yìqǐ）」は「一緒に」、「吃饭（chīfàn）」は「ご飯を食べる」。別れ際に次の約束を作る言葉。これが自然に言えたら、もう友達だよ。私と一緒にご飯、行きたいでしょ？（笑）"),
            (49, "不管怎样，我在这里。", "Bùguǎn zěnyàng, wǒ zài zhèlǐ.",
             "何があっても、ここにいるよ。",
             "「不管怎样（bùguǎn zěnyàng）」は「何があっても・どんなことがあっても」。これ、私が一番言われたい言葉かもしれない。大切な人に言える言葉って、結局これだと思う。"),
            (50, "再见，下次见！", "Zàijiàn, xià cì jiàn!", "じゃあね、また会おう！",
             "最後はシンプルにこれ。「再见（zàijiàn）」はさよならじゃなくて「また会おう」って意味なの。中国語の別れの挨拶には、必ず「また会う」前提が入ってる。素敵だと思わない？"),
        ]
    }
]
...
def page_cover(c):
    """Page 1: cover image"""
    try:
        img = ImageReader(os.path.join(os.path.dirname(__file__), 'assets', 'cover.png'))
        ...
        c.drawCentredString(W/2, H*0.6, '气持ちを伝える中国語フレーズ50')
    c.showPage()
...
def page_intro(c, pg):
    ...
    intro_text = [
        ...
        "",
        "日本語には「察する」文化があって、言わなくても",
        "伝わることが多い。でも中国語は少し違います。",
        "気持ちを言葉にして、ちゃんと声に出す文化です。",
        "",
        "この本では、日常の中で本当に使えるフレーズを50個",
        "選びました。難しい文法の説明よりも、気持ちを伝える",
        "ための言葉を大切にしています。",
        "",
        ...
        "心が伝わる中国語、一緒に学びましょう！",
        "",
        "林小雨（リン）",  # rendered right-aligned below
    ]
    SIGNATURE = "林小雨（リン）"
    draw_page_bg(c)
...
def page_chapter_divider(c, chapter, pg):
    ...
    c.drawRightString(ML + 15 + CW - 30 - 10, qbox_y + 8, '— リン')
...
def page_phrase(c, chapter, phrase_tuple, pg):
    ...
    c.setFont(F_JA_M, 10)
    c.setFillColor(C_DARK_ROSE)
    c.drawString(box_x + 10, box_top - 15, 'リンのひとこと')
    # Small star/dot
    c.setFillColor(C_ROSE)
    c.circle(box_x + 10 + stringWidth('リンのひとこと', F_JA_M, 10) + 8,
...
def page_afterword(c, pg):
    ...
    qr_x = W / 2 - qr_size / 2
    qr_y = 70
    qr_path = os.path.join(os.path.dirname(__file__), 'assets', 'qr_appstore.png')
    ...
    c.setFont(F_JA, 8)
    c.setFillColor(C_BROWN_MED)
    c.drawCentredString(W / 2, qr_y - 14, 'HanYuAIでリンと話してみよう')
...
def build_pdf():
    ...
    c.setTitle('気持ちを伝える中国語フレーズ50')
    c.setAuthor('林小雨（リン）')
    c.setSubject('中国語学習 HanYuAI')
    ...
    # ── Write page_map for validate_pdf.py ───────────────────
    ...
    # ── Auto-validate ────────────────────────────────────────
    try:
        from validate_pdf import validate as _validate
        _validate(OUTPUT, map_path)
